...
layout(set = 0, binding = 3) uniform samplerCubeArrayShadow shadowCubeArray;
...
float samplePointLightShadowPcf(samplerCubeArrayShadow cube, vec3 sampleDir, float depthCompare, int cubeIndex) {
    vec3 L = sampleDir;
    vec3 up = mix(vec3(1.0, 0.0, 0.0), vec3(0.0, 1.0, 0.0), step(0.999, abs(L.y)));
    ...
    float pcfStep = texelAngle * PCF_STEP_TEXELS;
    float diskRadius = 2.8284271247461903 * pcfStep;
    ...
    for (int k = 0; k < PCF_SAMPLES; k++) {
        ...
        sum += texture(cube, vec4(normalize(off), float(cubeIndex)), depthCompare);
    }
    return sum / float(PCF_SAMPLES);
}

vec4 CalcLightResult(vec4 lightPos, vec4 lightCol) {
    vec3 norm = normalize(fragNormal);
    vec3 lightDir = normalize(lightPos.xyz - fragWorldPos);
    float diff = max(dot(norm, lightDir), 0.0);
    return diff * lightCol;
}

void main() {
    vec4 texColor = texture(texSampler[textureIndex], fragTexCoord);
    vec3 n = normalize(fragNormal);
    vec4 normalViz = vec4(n * 0.5 + 0.5, 1.0);
    ...
    for (int i = 0; i < 32; i++) {
        ...
        float shadowMask = step(0.0, float(ubo.lights[i].shadow_cube_index));
        int cubeIdx = max(0, ubo.lights[i].shadow_cube_index);

        vec3 toLight = fragWorldPos - ubo.lights[i].light_pos.xyz;
        float dist = length(toLight);
        vec3 sampleDir = normalize(toLight);
        sampleDir.y *= (-1.0);

        float sn = ubo.shadow_near;
        float sf = ubo.shadow_far;
        float depthRange = sf - sn;
        float depthLinear = (dist - sn) / depthRange;

        vec3 N = normalize(fragNormal);
        vec3 LtoSurf = normalize(ubo.lights[i].light_pos.xyz - fragWorldPos);
        ...
        float shadowFactor = samplePointLightShadowPcf(shadowCubeArray, sampleDir, depthCompare, cubeIdx);
        lightContrib *= mix(1.0, shadowFactor, shadowMask);
    ...
    vec4 litFinal = finalLight * texColor;
    outColor = mix(litFinal, normalViz, step(0.5, ubo.render_mode));
}
